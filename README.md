1. 프로젝트 목표 설정
* 목적: PointTransformerV3와 MLP 모델을 병합하여 실시간 포인트 클라우드 세그멘테이션과 오도메트리 추정을 수행할 모델을 구축할 것
* 입출력 정의: 원시 포인트 클라우드 데이터 [N, 6] (x, y, z, nx, ny, nz)를 입력으로 받아 최종 출력은 위치 (x, y, z)와 쿼터니언 (q1, q2, q3, q4)으로 표현된 포즈 * 행렬을 추정할 것
2. 데이터 전처리 및 통합
* PointTransformerV3 전처리: 위치 인코딩과 패치화 과정을 포함한 PointTransformerV3의 포인트 클라우드 전처리 방식을 유지하여 특징을 추출할 것
* Pose 데이터 표준화: MLP 입력에 맞게 pose 데이터 변화량을 표준화할 것
* 세그멘테이션 라벨 결합: Scan1과 Scan2의 특징 벡터 [N, 512]와 세그멘테이션 라벨을 concat하여 MLP 입력으로 사용할 것
3. 모델 구조 설계
* 트랜스포머 + MLP 병합: PointTransformerV3 특징 벡터와 라벨을 결합하여 MLP의 두 가지 브랜치(Rotation, Translation)로 입력할 것
* MLP 모델 분기: MLP는 각각 회전(rotation)과 이동(translation)을 예측하는 두 개의 브랜치로 구성할 것
* 손실 계산: Rotation loss와 Translation loss를 합산하여 총 손실(Total loss)을 계산하고 Back Propagation 수행
4. 메모리 최적화 및 성능 개선
* 배치 크기 조정: RTX8000 GPU 메모리 한계를 고려하여 배치 크기를 조정할 것
* 점진적 처리 방식: [N, 512] 특징 벡터 저장 시 메모리 효율성을 위해 점진적으로 처리할 것
* GPU 메모리 모니터링: 학습 중 GPU 메모리 사용량을 모니터링하며 최적화할 것
5. 출력 형식 및 손실 함수 설계
* 출력 형식 정의: 포즈 행렬 (x, y, z, q1, q2, q3, q4) 형식을 정의하여 추정 결과 일관성 확보
* 손실 함수 설계: 세그멘테이션 결과와 포즈 추정을 모두 고려하여 손실 함수를 설계할 것
* 실시간 추론 파이프라인 구성: 실시간 성능을 위해 출력 파이프라인을 효율적으로 구성할 것
6. 실시간 성능 테스트 및 최적화
* 목표 FPS 달성 확인: 모델이 목표 FPS(15FPS)를 달성하는지 테스트할 것
* 병목 제거: 추론 시 병목 구간을 파악하고 최적화할 것
* 프로파일링 및 최적화: 각 레이어의 시간 소모를 분석하고 병목이 되는 부분을 최적화할 것
7. 최종 검증 및 배포 준비
* 최종 성능 검증: 목표 성능을 검증하고 단계별로 성능 테스트를 진행할 것
* 배포 준비: 배포 환경에 맞추어 모델을 최적화하고 실시간 시스템에 통합할 것
